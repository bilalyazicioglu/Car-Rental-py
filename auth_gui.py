...
class AuthWindow(tk.Toplevel):
    ...
    def _build(self):
        frame = tk.Frame(self, padx=30, pady=30, bg=BG_COLOR)
        frame.pack(expand=True)

        tk.Label(frame, text="Kullanıcı Adı", bg=BG_COLOR, fg=FG_COLOR).pack(anchor="w")
        tk.Entry(frame, textvariable=self.username, bg='#2d2d3f', fg=FG_COLOR, insertbackground=FG_COLOR).pack(fill="x")

        tk.Label(frame, text="Şifre", bg=BG_COLOR, fg=FG_COLOR).pack(anchor="w", pady=(10, 0))
        tk.Entry(frame, textvariable=self.password, show="*", bg='#2d2d3f', fg=FG_COLOR, insertbackground=FG_COLOR).pack(fill="x")

        # Giriş butonu (Label tabanlı - macOS uyumlu)
        login_btn = tk.Label(frame, text="Giriş Yap", bg=ACCENT, fg=FG_COLOR, 
                            font=("Helvetica", 11, "bold"), pady=8, cursor="hand2")
        login_btn.pack(fill="x", pady=(15, 5))
        login_btn.bind("<Button-1>", lambda e: self.login())

        # Kayıt butonu
        register_btn = tk.Label(frame, text="Kayıt Ol", bg='#2d2d3f', fg=FG_COLOR,
